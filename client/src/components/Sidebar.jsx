...
const Sidebar = () => {
    ...
    const fileteredUsers = input? users.filter((user)=>user.fullName.toLowerCase().includes(input.toLowerCase())) : users;
        ...
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll scrollbar-hide text-white ${selectedUser ? "max-md:hidden" : ""}`}>
            <div className="pb-5">
                ...
                <div className='bg-[#282142] rounded-full flex items-center gap-2 mt-5 py-3 px-4'>
                    <img src={assets.search_icon} alt="screen" className='w-3' />
                    <input onChange={(e)=>setInput(e.target.value)} type="text" placeholder='Search User..' className='bg-transparent outline-none border-none text-white text-xs placeholder-[#c8c8c8] flex-1' />
                </div>
            </div>
            <div className='flex flex-col'>
                {fileteredUsers.map((user, index) => (
                    ...
                    ({...prev, [user._id]:0}))
                        ...
                        <img src={user?.profilePic || assets.avatar_icon} alt="profile" className='w-[35px] aspect-[1/1] h-10 rounded-full' />
                        <div className='flex flex-col'>
